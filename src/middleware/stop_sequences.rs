...
pub async fn apply_stop_sequences(resp: Response) -> Response {
    ...
    let mut positions = vec![Position::from(search)];
    let mut stop = false;
    let stream = stream.flat_map(move |event| {
        if stop {
            return stream::iter(vec![]);
        }
        let mut searches = positions
            .iter()
            .map(|p| IncSearch::resume(&trie, *p))
            .collect::<Vec<_>>();
        ...
        positions = searches.into_iter().map(Position::from).collect();
